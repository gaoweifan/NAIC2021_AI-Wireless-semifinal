...
num_heads=8
num_layers=4
transformer_units=[4096,embedding_dim]
def InitConv(x):
...
def rxModel(input_bits):
    ...
    temp=EnDown(temp,base_channels*4,(2,1,1)) # (64,16,2,64)

    x3=EnBlock(temp,base_channels*4)
    temp=EnBlock(x3,base_channels*4)
    temp=EnDown(temp,base_channels*8,(1,1,1)) # (64,16,2,128)

    x4=EnBlock(temp,base_channels*8)
    temp=EnBlock(x4,base_channels*8)
    temp=EnBlock(temp,base_channels*8)
    temp=EnBlock(temp,base_channels*8)        # (64,16,2,128)

    temp = layers.BatchNormalization()(temp)
    temp = layers.Activation('relu')(temp)
    temp = layers.Conv3D(embedding_dim, (3, 3, 3), padding='same')(temp)# (64,16,2,512)
    temp = layers.Reshape((-1,embedding_dim))(temp)                     # (2048,512)

    for _ in range(num_layers):
        temp = transformerBlock(temp)

    # decoder
    temp = layers.Reshape((64,16,2,embedding_dim))(temp)                # (64,16,2,512)
    temp = DeBlock1(temp,base_channels*8)
    temp = DeBlock(temp,base_channels*8)                                # (64,16,2,128)

    temp = DeUp_Cat(temp,x3,base_channels*4,(1,1,1))                    # (64,16,2,64)
    temp = DeBlock(temp,base_channels*4)

    temp = DeUp_Cat(temp,x2,base_channels*2,(2,1,1))                    # (128,16,2,32)
    ...
    temp = layers.BatchNormalization()(temp)
    temp = layers.Activation('relu')(temp)
    temp = layers.Conv3D(2, (3, 3, 3), strides=(1,2,1), padding='same')(temp)# (256,4,2,2)
    temp = layers.BatchNormalization()(temp)
    temp = layers.Activation('relu')(temp)
