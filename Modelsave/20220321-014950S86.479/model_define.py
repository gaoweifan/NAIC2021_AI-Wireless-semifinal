...
def rxModel(input_bits,trainable=True):
    ...
    temp = Mlp([mlp_num,64], drop=0.,trainable=trainable,name="mid_mlp")(x_final)
    x0 = layers.Reshape((256,2,16*2))(temp)
    x1 = layers.Conv2D(32, (3, 3), padding='same', name="conv2d_out_1",trainable=trainable)(x0)
    temp = layers.BatchNormalization(trainable=trainable)(x1)
    temp = layers.LeakyReLU(alpha=0.1)(temp)

    x2 = layers.Conv2D(128, (3, 3), padding='same', name="conv2d_out_2",trainable=trainable)(temp)
    temp = layers.BatchNormalization(trainable=trainable)(x2)
    temp = layers.LeakyReLU(alpha=0.1)(temp)

    x3 = layers.Conv2D(32, (3, 3), padding='same', name="conv2d_out_3",trainable=trainable)(temp)
    temp = layers.BatchNormalization(trainable=trainable)(x3)
    temp = layers.LeakyReLU(alpha=0.1)(temp)
    ...
    # Add MLP.
    features = Mlp([mlp_num,8], drop=0.,trainable=trainable,name="output_mlp")(temp)
