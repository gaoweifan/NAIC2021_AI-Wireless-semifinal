...
def rxModel(input_bits,trainable=True):
    ...
    x1 = layers.BatchNormalization()(temp)

    x2 = layers.Conv2D(32, (3, 3), padding='same', name="conv2d_in_1")(x1)
    temp = layers.BatchNormalization()(x2)
    temp = layers.LeakyReLU(alpha=0.1)(temp)

    x3 = layers.Conv2D(128, (3, 3), padding='same', name="conv2d_in_2")(temp)
    temp = layers.BatchNormalization()(x3)
    temp = layers.LeakyReLU(alpha=0.1)(temp)

    x4 = layers.Conv2D(32, (3, 3), padding='same', name="conv2d_in_3")(temp)
    temp = layers.BatchNormalization()(x4)
    temp = layers.LeakyReLU(alpha=0.1)(temp)
