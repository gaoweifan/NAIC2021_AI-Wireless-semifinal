...
def rxModel(input_bits):
    ...
    # temp=EnDown(x1,base_channels*2,(2,1,1)) # (128,16,2,32)

    # x2=EnBlock(temp,base_channels*2)
    # temp=EnBlock(x2,base_channels*2)
    # temp=EnDown(temp,base_channels*8,(2,2,1)) # (64,8,2,128)

    # x3=EnBlock(temp,base_channels*8)
    # temp=EnBlock(x3,base_channels*8)
    # temp=EnDown(temp,base_channels*16,(4,2,2)) # (16,4,1,256)

    # x4=EnBlock(temp,base_channels*16)
    # temp=EnBlock(x4,base_channels*16)
    # temp=EnBlock(temp,base_channels*16)
    # temp=EnBlock(temp,base_channels*16)        # (16,4,1,256)

    # temp = layers.BatchNormalization()(temp)
    # temp = layers.Activation('relu')(temp)
    # temp = layers.Conv3D(embedding_dim, (3, 3, 3), padding='same')(temp)# (16,4,1,512)
    ...
    # # decoder
    # temp = layers.Reshape((16,4,1,embedding_dim))(temp)                # (16,4,1,512)
    # temp = DeBlock1(temp,base_channels*16)
    # temp = DeBlock(temp,base_channels*16)                                # (16,4,1,256)

    # temp = DeUp_Cat(temp,x3,base_channels*8,(4,2,2))                    # (64,8,2,128)
    # temp = DeBlock(temp,base_channels*8)

    # temp = DeUp_Cat(temp,x2,base_channels*2,(2,2,1))                    # (128,16,2,32)
    # temp = DeBlock(temp,base_channels*2)

    # temp = DeUp_Cat(temp,x1,base_channels,(2,1,1))                      # (256,16,2,16)
    ...
    # temp = layers.BatchNormalization()(temp)
    # temp = layers.Activation('relu')(temp)
    # temp = layers.Conv3D(2, (3, 3, 3), strides=(1,4,1), padding='same',name="test")(temp)# (256,4,2,2)
    # temp = layers.BatchNormalization()(temp)
    # temp = layers.Activation('relu')(temp)
    ...
    out_put = layers.Dense(4*256*2, activation="sigmoid")(temp)
