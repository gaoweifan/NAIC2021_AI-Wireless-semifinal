...
def rxModel(input_bits,trainable=True):
    ...
    temp = layers.Conv2D(32, (3, 3), padding='same', name="conv2d_in_1")(temp)
    temp = layers.BatchNormalization()(temp)
    temp = layers.LeakyReLU(alpha=0.1)(temp)

    temp = layers.Conv2D(128, (3, 3), padding='same', name="conv2d_in_2")(temp)
    temp = layers.BatchNormalization()(temp)
    temp = layers.LeakyReLU(alpha=0.1)(temp)

    temp = layers.Conv2D(32, (3, 3), padding='same', name="conv2d_in_3")(temp)
    temp = layers.BatchNormalization()(temp)
    temp = layers.LeakyReLU(alpha=0.1)(temp)
