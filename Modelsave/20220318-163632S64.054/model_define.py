...
def rxModel(input_bits,trainable=True):
    ...
    temp = layers.Conv3D(32, (3, 3, 3),strides=(1,2,1), padding='same', name="conv3d_in_1",trainable=trainable)(temp)#256*1*2*32
    temp = layers.BatchNormalization(trainable=trainable)(temp)
    temp = layers.Activation('relu')(temp)
    temp = layers.Permute((1,3,4,2))(temp)#256(载波)*2(IQ)*32*1
    temp = layers.Reshape((256,2,32))(temp)

    temp = layers.Conv2D(64, (7, 7), strides=(2, 1), padding='same', name="conv2d_in_2",trainable=trainable)(temp)#128*2*64
    temp = layers.BatchNormalization(trainable=trainable)(temp)
    temp = layers.Activation('relu')(temp)
    temp = layers.Permute((1,3,2))(temp)#128*64*2
    temp = layers.Conv2D(4, (7, 7), strides=(2, 1), padding='same', name="conv2d_in_3",trainable=trainable)(temp)#64*64*4
    temp = layers.BatchNormalization(trainable=trainable)(temp)
    temp = layers.Activation('relu')(temp)

    temp = transunet_2d_base(temp, [8, 16, 32, 64], stack_num_down=2, stack_num_up=2, 
                          embed_dim=768, num_mlp=768, num_heads=3, num_transformer=3,
                          activation='ReLU', mlp_activation='GELU', batch_norm=True, pool=True, unpool='bilinear',
                          backbone=None, weights=None, freeze_backbone=True, freeze_batch_norm=False, name="transunet")

    temp = layers.Conv2D(4, (7, 7), strides=(1, 1), padding='same', name="conv2d_out_3",trainable=trainable)(temp)#256,2,4
    temp = layers.BatchNormalization(trainable=trainable)(temp)
    temp = layers.Activation('relu')(temp)
    # temp = layers.Permute((3,1,2))(temp)#4,256,2

    temp = layers.Flatten()(temp)
    out_put = layers.Dense(4*256*2, activation='sigmoid',trainable=trainable)(temp)# 4*256*2 bit
    return out_put